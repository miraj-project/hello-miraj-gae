...
(def gae
  ...
  {;; :build-dir ; default: "build";  gradle compatibility: "build/exploded-app"
   ...
   :servlets [{:ns 'hello.miraj.greeting
               :name "main-servlet"
               :desc {:text "greeting servlet - calls mirajified webpage"}
               :url "/miraj/*"}

              {:ns 'hello.miraj.echo  ;; = servlet-class
              ...
              {:ns 'hello.miraj.math      ;; REQUIRED
               :name "math-servlet"  ;; REQUIRED
               :url "/math/*"      ;; REQUIRED
               :params [{:name "op" :val "+"}
                        {:name "arg1" :val 3}
                        {:name "arg2" :val 2}]}]
   :filters [{:ns 'hello.miraj.reloader   ; REQUIRED
